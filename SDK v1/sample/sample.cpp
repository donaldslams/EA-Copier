...
void callback_sample() {
    ...
    sym.SecType = 5;
    ret = PlaceMarketOrder((char*)account, SELL, 1000, sym);
    if (ret != 0) {
        std::cout << GetLastErrorMessage() << std::endl;
        return;
    }   
    ...
    while(true) {
        ...
        ret = PlaceLimitOrder((char*)account, BUY, 1000, sym, 1.2);
        if (ret != 0) {
            std::cout << GetLastErrorMessage() << std::endl;
            return;
        }
        ...
        std::this_thread::sleep_for(std::chrono::seconds(1));
        ret = PlaceStopOrder((char*)account, BUY, 1000, sym, 1.2);
        if (ret != 0) {
            std::cout << GetLastErrorMessage() << std::endl;
            return;
        }
...
//    for (int i = 0; i<100; i++) {
    while(true){
        ...
        { 
            ...
            if (counter % 3 == 0) {
                ...
                if (order->Sym != nullptr) {
                    // (char*)account, (char*)account, (char*)("1"), 1, SELL, 1000, sym, (char*)"INTX", (char*)"", 1
        ...
        std::this_thread::sleep_for(std::chrono::seconds(5));
        ret = PlaceLimitOrder((char*)account, BUY, 1000, sym, 1.0);
        if (ret != 0) {
          std::cout << GetLastErrorMessage() << std::endl;
          return;
        }
...
void polling_sample_2() {
    ...
    std::cout << "Call PlaceLimitOrder " << std::endl;
    //std::this_thread::sleep_for(std::chrono::seconds(5));
    ret = PlaceLimitOrderEx((char*)account, BUY, 1000, symName, SECURITY_TYPE::SecurityTypeFOR, 1.0);
    if (ret != 0) {
      std::cout << GetLastErrorMessage() << std::endl;
      return;
    }
    ...
    std::cout << "Call PlaceStopOrder " << std::endl;
    //std::this_thread::sleep_for(std::chrono::seconds(5));
    ret = PlaceStopOrderEx((char*)account, BUY, 1000, symName, SECURITY_TYPE::SecurityTypeFOR, 1.2);
    if (ret != 0) {
      std::cout << GetLastErrorMessage() << std::endl;
      return;
    }
    ...
    ret = PlaceThresholdOrderEx((char*)account, BUY, 1000, symName, SECURITY_TYPE::SecurityTypeFOR, 1.2, 1.0);
    if (ret != 0) {
      std::cout << GetLastErrorMessage() << std::endl;
      return;
    }
    ...
    while (counter++ < 4) {
        ...
        while (true) {
          ...
          case OBJ_TYPE_ORDER: {
            Order* order = (Order*)msg.Message;
            if (order->ExecType == EXEC_TYPE::ExecTypeNew) { // this should be '0' not 0
        ...
        case OBJ_TYPE_EXECUTION_ALL_DATA: {
            ...
            std::cout << "Call CancelOrder " << std::endl;
/*
            Order* order = data->OrdInfo; // the order is already executed
            if(order != nullptr) {  
                Symbol sym;
                sym.Name = nullptr;
                sym.Tenor = nullptr;
